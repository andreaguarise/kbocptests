...
def subscribePaaS (userName, kbUserId, subscriptionStart)
  requestBody = {
                                "accountId" => kbUserId,
  ...
  puts user["target"].to_s
    ...
    when type == "PaaS"
      if paasBundle["bundleId"]
        puts "PaaS bundle already exists for this user"
        puts paasBundle.to_s
      else
        puts "Creating PaaS bundle for user #{thisUser}"
        subscribePaaS(thisUser, kbparsed["accountId"], subscriptionStart)
      end
    end
  else
